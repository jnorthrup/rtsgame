 ...
 */
val EmptySeries: Series<Nothing> = j(0) { _: Int ->
...
fun <T> Collection<T>.toSeries(): Series<T> = 
    j(this.size) { index -> this.elementAt(index) }

/**
 * Creates a Series from an array.
 */
fun <T> Array<T>.toSeries(): Series<T> = 
    j(this.size) { index -> this[index] }
...
fun <T, R> Series<T>.map(transform: (T) -> R): Series<R> = 
    j(size(this)) { index -> transform(this[index]) }
...
fun <T> Series<T>.take(n: Int): Series<T> {
    ...
    return j(actualSize) { index -> this[index] }
}
...
fun <T> Series<T>.drop(n: Int): Series<T> {
    ...
    return j(newSize) { index -> this[index + actualStart] }